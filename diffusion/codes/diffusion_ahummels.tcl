...
proc diffusion {outfile infile axis abeg aend nT ts bin_size com_on} {

# Read in pore radii for each frame
set in [open $infile]
set data [read $in]
close $in
...
set rad_list {}
foreach l $lines {
	lappend rad_list [lindex $l 1]
	}

# Find maximum radius (for use with shells)
set max_rad [lindex $rad_list 0]
set len [llength $rad_list]
for {set i 1} {$i < $len} {incr i} {
	set rad0 [lindex $rad_list $i]
	if {$rad0 > $max_rad} {
		set $max_rad $rad0
	}
}
...
# Select pore and water molecules within the pore radius
set pore [atomselect top "protein and backbone and resid 31 to 37"]
$pore frame $abeg
set bx [expr {[lindex [measure center $pore] 0]}]
set by [expr {[lindex [measure center $pore] 1]}]
set rad [expr {[lindex $rad_list $abeg]}]

set sel [atomselect top "name OH2 and sqr(x - $bx) + sqr(y - $by) < sqr($rad) " frame $abeg]
set sel0 [atomselect top "name OH2 and sqr(x - $bx) + sqr(y - $by) < sqr($rad) " frame $abeg]
...
#--------------Procedure to create a list of lists of length dim--------#
proc llcreate {dim} {
	set v {}
	for {set i 0} {$i < $dim} {incr i} {
        	lappend v {}
...
#--------------Compute MSD for each time window, bin by distance, append results and average--------#
set shelled [shells $sel $pore 0 $max_rad $bin_size]
set MSD_len [llength $shelled]
set MSD [veccreate $MSD_len]
set norm_list [veccreate $MSD_len]

# Ask how norm should be modified 
# set norm [expr {$nummol*$nw}]
# Probably would have to do the normalization at the end / at every time step?
...
for {set j $abeg} {$j<$aend} {incr j $nT} {
	...
	for {set i 0} {$i<$nT} {incr i} {
        ...
        # Reorganize MSDs by bins, count number of atoms per bin
        $pore frame $j
	$pore update
	set shelled [shells $sel0 $pore $j $max_rad $bin_size]
	set sel_list [expr {[$sel0 get index]}]
	for {set i 0} {$i < $nummol} {incr i} {
		set k -1
		set idx -1
		set sel_id [lindex $sel_list $i]
		while {$idx < 0 && $k < $MSD_len} {
			set $k [expr {$k + 1}]
			set $idx [lsearch [lindex $shelled $k] $sel_id]
		}
		lset MSD $k [expr {[lindex $MSD $k] + [lindex $sdlist $i]}]
		lset norm_list $k [expr {[lindex $norm_list $k] + 1}]
	}
...
set MSD [vecscale $MSD [expr {1.0/$nummol}]]

for {set i 0} {$i<$MSD_len} {incr i} {
	if {expr {[lindex $MSD $i] != 0}} {
		lset MSD $i [expr {[lindex $MSD $i] / [lindex $norm_list $i]}]
	}
