...
mol new /home/sachin/Research/BioPhys/diffusion/data/2m4j/2M4J-structure1-double-solvate-new-ionized.psf waitfor all
mol addfile /home/sachin/Research/BioPhys/diffusion/data/2m4j/2m4j-inf-NPT5-11-water-around-prot-frames0-1000.dcd waitfor all
set molids {}
lappend molids [molinfo top]
align "protein and backbone" [lindex $molids 0]
mol new /home/sachin/Research/BioPhys/diffusion/data/2m4j/2M4J-structure1-double-solvate-new-ionized.psf waitfor all
mol addfile /home/sachin/Research/BioPhys/diffusion/data/2m4j/2m4j-inf-NPT5-11-unwrap-frames0-1000.dcd waitfor all
lappend molids [molinfo top]
align "protein and backbone" [lindex $molids 1]

set abeg 1
set aend 900
set wind_size_list "3 4 5"
set next_ref_list "23 24 25"
#set wind_size 500
#set next_ref 250
...
for {set nshell 1} {$nshell <= $nshells} {incr nshell} {
  foreach {j k} $wind_and_ref_list {
    ...
    set rIn2 [expr pow(10 - ($nshell-1)*$thickness,2)]
    set rOut2 [expr pow(10 - $nshell*$thickness,2)]
    ...
    if {$num_wind > 1} {
      ...
      file mkdir "shell${nshell}/tau${wind_size}f"
      set msdXfile "shell${nshell}/tau${wind_size}f/unwrap-diff-x_out.txt"
      set msdYfile "shell${nshell}/tau${wind_size}f/unwrap-diff-y_out.txt"
      set msdZfile "shell${nshell}/tau${wind_size}f/unwrap-diff-z_out.txt"
      set stdvXfile "shell${nshell}/tau${wind_size}f/stdev-x_out.txt"
      set stdvYfile "shell${nshell}/tau${wind_size}f/stdev-y_out.txt"
      set stdvZfile "shell${nshell}/tau${wind_size}f/stdev-z_out.txt"
